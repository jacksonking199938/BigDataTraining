...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    //从西游记小说中读取数据
    val rdd = sc.textFile("D:\\ACoder\\AllProject\\JavaProject\\BigDataTrain\\helloWorld\\西游记.txt")
      .map { x =>
    ...
    var result2 = result.filter(x => (x._1.equals("如来")||x._1.equals("孙悟空"))).reduce((x,y)=>(x._1+y._1,x._2+y._2))
    ...
    //输出所有结果到本文中
    sorted.map(x=>{x._1+","+x._2}).saveAsTextFile("D:\\ACoder\\AllProject\\JavaProject\\BigDataTrain\\helloWorld\\output\\"+System.nanoTime())
  }
}